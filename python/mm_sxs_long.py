...
if __name__ == "__main__":
    sxs_list_path = "../data_files/sxs/SXS_large_chip_Res_List_Long.txt"
    freq_list = "freqs.txt"
    ...
    mflag = mass_flag
    mass_str = ["M37.5", "M150","M75","M112.5","M187.5","M225"][mass_flag]
    mr_flag = int(sys.argv[2])
    fdata=np.genfromtxt(f"../data_files/sxs/{mass_str}_f_peak_long.txt",names=True)
    ...
    if approx_2_str == "SEOB":
        result = Parallel(n_jobs= num_procs)(
            delayed(utils.sxs_long_seobV5_mm)(i, data, inc, mr_flag, dist, maxfun, mflag,f_peaks) for i in range(list_length))
    if approx_2_str == "TEOB":
         result = Parallel(n_jobs= num_procs)(
            delayed(utils.sxs_long_teob_mm)(i, data, inc, mr_flag, dist, maxfun, mflag,f_peaks) for i in range(list_length))
    if approx_2_str == "TPHM":
         result = Parallel(n_jobs= num_procs)(
            delayed(utils.sxs_long_tphm_mm)(i, data, inc, mr_flag, dist, maxfun, mflag,f_peaks) for i in range(list_length))
    if approx_2_str == "XPHM":
         result = Parallel(n_jobs= num_procs)(
            delayed(utils.sxs_long_xphm_mm)(i, data, inc, mr_flag, dist, maxfun, mflag,f_peaks) for i in range(list_length))
